...
describe("Packages write", () => {
  it("Can create a new package", async () => {
    const response: request.Response = await request(server).post(prefix).send({
  ...
  it("Can update an existing package", async () => {
    const origResponse: request.Response = await request(server).get(`${prefix}/1`);
    ...
    const updatedResponse = await request(server).put(`${prefix}/${origData.id}`).send(newData)
...
describe("Packages delete", () => {
  it("Can delete an existing package", async () => {
    const delResponse: request.Response = await request(server).delete(`${prefix}/1`);
